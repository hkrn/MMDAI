...
VPVM として再開発、0.32.0 は欠番。技術的な変更点は以下

 - Qt 5.2 ベースに変更し、QML+QtQuick で UI を再設計
 - エフェクトシステムを開発が中止された CgFX から nvFX に変更
 - ライブラリの API の見直しと内部処理の大幅な変更

## 0.30.0 (2013/6/8)

0.29.x は 0.30.0 の開発版として採番されました

 - 仕様変更
  - エフェクトの実装の大幅な書き直し
  - PMX の IK 処理の変更
  - PMD の内部読み込み処理の変更
  - MMD 7.4x 以降で採用された新しい VMD 形式の暫定対応
 - 不具合修正
  - カメラモーションの読み込みが出来ない不具合の修正
  - 再生時の音源が再生されない不具合の修正
  - プロジェクト保存時アクセサリの情報が復元されない不具合の修正
  - グループモーフにグループモーフが存在する PMX モデルが読みない不具合の修正

## 0.28.2 (2013/3/23)

 - 新機能
  - 拡張ポーズ形式（ボーンだけでなくモーフ情報を含む）の入出力の対応
 - 仕様変更
  - zip 読み込み時 README を表示し、それに同意しないと読み込ませない様に変更
  - デフォルトの視野角を 30 から 27 に変更
  - ユニバーサルバイナリの復活
  - テクスチャローダの実装を変更
   - TGA 形式で読めないテクスチャが読めるようになる場合があります
   - 圧縮形式で読み込んだ時 DDS 形式のテクスチャが読めなくなります
 - 不具合修正
  - エフェクト有効時一部モデルで材質が黒く変色する不具合の修正
  - 画像出力を行うと次回以降画像出力が出来なくなる不具合の修正
  - ポストエフェクト有効時ウィンドウを変形（リサイズ）するとアスペクト比を無視した描画結果になる不具合の修正
