...
create table `effect_keyframes` (
  ...
  foreign key (parent_track_id) references effect_tracks (id) on update restrict on delete cascade,
  foreign key (parent_layer_id) references effect_layers (id) on update restrict on delete cascade
);
...
create view `motion_duration` as
...
create trigger `create_ik_parameters_at_inserting_bone_track` after insert on `bone_tracks`
  begin
    insert into `model_keyframe_ik_parameters` (`parent_track_id`) values (new.id);
  end;

insert into models (`uuid`, `order`, `name`) values ( '{00000000-0000-0000-0000-000000000000}', -1, 'NULL' );
insert into motions (`parent_model_id`, `uuid`)
  select last_insert_rowid(),
         ...
         from models
         where models.uuid = '{00000000-0000-0000-0000-000000000000}';
insert into camera_layers (`parent_motion_id`, `index`, `name`)
  select last_insert_rowid(), 0, 'base';
insert into camera_keyframes (`parent_layer_id`, `time_index`)
...
insert into motions (`parent_model_id`, `uuid`)
  select models.id,
         ...
         from models
         where models.uuid = '{00000000-0000-0000-0000-000000000000}';
insert into light_layers (`parent_motion_id`, `index`, `name`)
  select last_insert_rowid(), 0, 'base';
insert into light_keyframes (`parent_layer_id`, `time_index`)
  select last_insert_rowid(), 0;
insert into motions (`parent_model_id`, `uuid`)
  select last_insert_rowid(),
         printf('{%s-%s-%s-%s-%s}', lower(hex(randomblob(4))), lower(hex(randomblob(2))), lower(hex(randomblob(2))), lower(hex(randomblob(2))), lower(hex(randomblob(6))))
         from models
         where models.uuid = '{00000000-0000-0000-0000-000000000000}';
insert into project_layers (`parent_motion_id`, `index`, `name`)
  select last_insert_rowid(), 0, 'base';
insert into project_keyframes (`parent_layer_id`, `time_index`)
  select last_insert_rowid(), 0;
