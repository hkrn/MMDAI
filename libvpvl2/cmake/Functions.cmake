...
function(vpvl2_find_assimp)
  if(VPVL2_LINK_ASSIMP)
    ...
    find_library(ASSIMP_LIBRARY assimp PATHS ${ASSIMP_LIBRARY_LOCAL_DIR} ${ASSIMP_LIBRARY_LOCAL_DIR2} $ENV{ASSIMP_LIBRARY_DIR})
  endif()
endfunction()
...
function(vpvl2_find_vpvl)
  if(VPVL2_LINK_VPVL)
    ...
    get_build_directory(VPVL_BUILD_DIRECTORY VPVL_SOURCE_DIRECTORY OFF)
    ...
    find_path(VPVL_CONFIG_DIR vpvl/config.h PATHS "${VPVL_BUILD_DIRECTORY}/include" $ENV{VPVL_CONFIG_DIR})
...
function(vpvl2_find_nvtt)
  if(VPVL2_ENABLE_EXTENSIONS_RENDERCONTEXT AND VPVL2_LINK_NVTT)
    ...
    # for MSVC
    get_local_library_directory_named(NVTT_NVCORE_LOCAL_DIR NVTT_SOURCE_DIRECTORY "src/nvcore")
    get_local_library_directory_named(NVTT_NVIMAGE_LOCAL_DIR NVTT_SOURCE_DIRECTORY "src/nvimage")
    get_local_library_directory_named(NVTT_NVMATH_LOCAL_DIR NVTT_SOURCE_DIRECTORY "src/nvmath")
    find_path(NVTT_CONFIG_INCLUDE_DIR nvconfig.h PATHS "${NVTT_BUILD_DIRECTORY}/src" $ENV{NVTT_CONFIG_INCLUDE_DIR})
    ...
    find_library(NVTT_NVCORE_LIBRARY nvcore PATHS ${NVTT_LIBRARY_LOCAL_DIR} ${NVTT_NVCORE_LOCAL_DIR} $ENV{NVTT_LIBRARY_DIR})
    find_library(NVTT_NVIMAGE_LIBRARY nvimage PATHS ${NVTT_LIBRARY_LOCAL_DIR} ${NVTT_NVIMAGE_LOCAL_DIR} $ENV{NVTT_LIBRARY_DIR})
    find_library(NVTT_NVMATH_LIBRARY nvmath PATHS ${NVTT_LIBRARY_LOCAL_DIR} ${NVTT_NVMATH_LOCAL_DIR} $ENV{NVTT_LIBRARY_DIR})
...
function(vpvl2_find_icu)
  if(VPVL2_ENABLE_EXTENSIONS_STRING)
    get_source_directory(ICU_SOURCE_DIRECTORY "icu-src")
    set(ICU_LIBRARY_LOCAL_DIR "${ICU_SOURCE_DIRECTORY}/lib")
    set(ICU_INCLUDE_UC_LOCAL_DIR "${ICU_SOURCE_DIRECTORY}/source/common")
    set(ICU_INCLUDE_I18N_LOCAL_DIR "${ICU_SOURCE_DIRECTORY}/source/i18n")
...
function(vpvl2_find_libxml2)
  if(VPVL2_ENABLE_EXTENSIONS_PROJECT)
    ...
    get_build_directory(LIBXML2_BUILD_DIRECTORY LIBXML2_SOURCE_DIRECTORY ON)
    if(MSVC)
      find_library(LIBXML2_LIBRARY libxml2_a PATH "${LIBXML2_SOURCE_DIRECTORY}/win32/bin.msvc")
    else()
      find_library(LIBXML2_LIBRARY xml2 PATH "${LIBXML2_BUILD_DIRECTORY}/lib")
    endif()
