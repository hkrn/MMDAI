...
void main() {
    ...
    float diffuse = max(dot(light, normal), zero);
    ...
    if (diffuse != zero) {
        ...
        // float specular = max(dot(normal, halfway), zero);
        float specular = pow(max(dot(normal, halfway), zero), gl_FrontMaterial.shininess);
